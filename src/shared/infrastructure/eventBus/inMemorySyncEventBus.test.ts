...
describe("inMemoryAsyncEventBus", () => {
  ...
  describe("subscribe", () => {
    it("should call a subscriber that is manually registered when the event it is subscribed to is published", async () => {
      expect.assertions(1);
  ...
  describe("unsubscribe", () => {
    it("should not call a subscriber that has been unsubscribed", async () => {
      expect.assertions(0);

      const event = new ExampleEvent({ id: ObjectMother.uuid().value });
      const subscriber = new DomainEventSubscriberExample();
      subscriber.setExpectation((_: ExampleEvent) => {
