...
test("should complete the happy path", async ({ page, isMobile }) => {
  ...
  await page.locator("#Revenue-1-amount").click();
  await page.locator("#Revenue-1-amount").fill("1000");
  await page.locator("#Revenue-1-name").click();

  await expect(page.locator("#Revenue-1-amount")).toHaveValue("$1,000");
  ...
  await page.getByLabel("item 2 amount").fill("50");
  await page.getByLabel("item 2 amount").press("Tab");

  await expect(page.getByLabel("item 2 name")).toHaveValue("sale");
  await expect(page.getByLabel("item 2 amount")).toHaveValue("$50");
  ...
  await page.getByLabel("item 3 amount").fill("25");
  await page.getByLabel("item 3 amount").press("Tab");
  ...
  await expect(page.getByLabel("item 3 amount")).toHaveValue("$35");
  ...
  await page.locator("#Expenses-1-amount").press("Tab");
  ...
  await page.locator("#Expenses-2-amount").fill("100");
  await page.locator("#Expenses-2-amount").press("Tab");
  ...
  await expect(page.locator("#Expenses-2-amount")).toHaveValue("$100");

  await page.getByLabel("add item to Expenses").press("Enter");
  await page.locator("#Expenses-3-name").fill("item");
  await page.locator("#Expenses-3-name").press("Tab");
  await page.locator("#Expenses-3-amount").fill("50");

  await expect(page.locator("#Expenses-3-name")).toHaveValue("item");
  await expect(page.locator("#Expenses-3-amount")).toHaveValue("$50");
  ...
  await page.getByLabel("apply change to item amount").click();

  await expect(page.locator("#Expenses-3-amount")).toHaveValue("$0");

  await page.getByLabel("add item to Expenses").click();
  await page.getByLabel("item 4 name").fill("mistake");
  await page.getByLabel("item 4 amount").click();
  await page.getByLabel("item 4 amount").fill("20");
  await expect(page.locator("#Expenses-4-name")).toHaveValue("mistake");
  await expect(page.locator("#Expenses-4-amount")).toHaveValue("$20");
  ...
  await expect(page.locator("#Expenses-4-amount")).not.toBeVisible();
